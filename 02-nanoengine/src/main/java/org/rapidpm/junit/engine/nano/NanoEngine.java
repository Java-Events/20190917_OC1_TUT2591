...
public class NanoEngine
  ...
  protected static Predicate<Class<?>> isTestClass() {
    return classCandidate -> match(
        matchCase(() -> failure("this class is not a supported by this TestEngine - " + classCandidate.getSimpleName())),
        matchCase(() -> isAbstract(classCandidate),
                  () -> failure("no support for abstract classes" + classCandidate.getSimpleName())),
        matchCase(() -> isPrivate(classCandidate),
                  () -> failure("no support for private classes" + classCandidate.getSimpleName())),
        matchCase(() -> isAnnotated(classCandidate, NanoTestClass.class),
                  () -> success(Boolean.TRUE))).ifFailed(msg -> Logger.getLogger(NanoEngine.class)
                                                                      .info(msg))
                                               .ifPresent(b -> Logger.getLogger(NanoEngine.class)
                                                                     .info("selected class " + classCandidate))
...
//    return () -> EngineDiscoveryRequestResolver
//        .builder()
...
//                  parent -> Optional.of(new NanoEngineClassTestDescriptor(testClass, parent.getUniqueId())))
